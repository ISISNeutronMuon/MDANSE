...
from Databases import NUCLEOTIDES, RESIDUES
from FortranFormat import FortranFormat, FortranLine
from Geometry import Vector, Tensor
    ...
    def nextResidue(self, name, number = None, terminus = None):
        ...
        if self.export_filter is not None:
            name, number = self.export_filter.processResidue(name, number,
                                                             terminus)
        self.het_flag =  not (name in RESIDUES or name in NUCLEOTIDES)
...
class PDBAminoAcidResidue(PDBResidue):
    ...
    def isNTerminus(self):
        ...
        @rtype: C{bool}
        """
        return '1HT' in self.atoms or \
               '2HT' in self.atoms or \
               '3HT' in self.atoms or \
               'HT1' in self.atoms or \
               'HT2' in self.atoms or \
               'HT3' in self.atoms or \
               '1H' in self.atoms or \
               '2H' in self.atoms or \
               '3H' in self.atoms or \
               'H1' in self.atoms or \
...
class PDBPeptideChain(PDBChain):
    ...
    def isCompatible(self, chain_data, residue_data):
        return chain_data['chain_id'] == self.chain_id and \
               ...
               residue_data['residue_name'] in RESIDUES
...
class PDBNucleotideChain(PDBChain):
    ...
    def isCompatible(self, chain_data, residue_data):
        return chain_data['chain_id'] == self.chain_id and \
               chain_data['segment_id'] == self.segment_id and \
               residue_data['residue_name'] in NUCLEOTIDES
...
class PDBDummyChain(PDBChain):
    ...
    def isCompatible(self, chain_data, residue_data):
        return chain_data['chain_id'] == self.chain_id and \
               chain_data['segment_id'] == self.segment_id and \
               residue_data['residue_name'] not in RESIDUES and \
               residue_data['residue_name'] not in NUCLEOTIDES
...
class Structure:
    ...
    def newChain(self, residue, chain_data):
        if hasattr(residue, 'is_amino_acid'):
            ...
            chain = PDBDummyChain(self, chain_data['chain_id'],
                               chain_data['segment_id'])
        return chain
