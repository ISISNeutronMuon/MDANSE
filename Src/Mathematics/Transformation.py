...
class Translation(RigidBodyTransformation):
    ...
    def screwMotion(self):
        ...
        if l == 0.:
            ...
            return Vector(0.,0.,0.), self.vector/l, 0., l
...
class Rotation(RigidBodyTransformation):
    ...
    def threeAngles(self, e1, e2, e3, tolerance=1e-7):
        ...
        if np.add.reduce(solutions[0]**2) > \
               np.add.reduce(solutions[1]**2):
    ...
    def asQuaternion(self):
        ...
        return Quaternion(cos_angle_2, sin_angle_2*axis[0],sin_angle_2*axis[1], sin_angle_2*axis[2])
    ...
    def screwMotion(self):
        axis, angle = self.axisAndAngle()
        return Vector(0., 0., 0.), axis, angle, 0.
...
class RotationTranslation(RigidBodyTransformation):
    ...
    def screwMotion(self):
        ...
        return r0, axis, angle % (2.*np.pi), d
...
if __name__ == '__main__':
    ...
    t = Translation(Vector(1.,-2.,0))
    r = Rotation(Vector(0.1, -2., 0.5), 1.e-10)
    q = r.asQuaternion()
    angles = r.threeAngles(Vector(1., 0., 0.),Vector(0., 1., 0.),Vector(0., 0., 1.))
    c = t*r
    print(c.screwMotion())
    s = Scaling(2.)
    all = s*t*r
