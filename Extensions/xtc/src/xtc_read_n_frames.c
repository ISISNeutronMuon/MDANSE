...
int read_xtc_n_frames(char *fn, int *n_frames, int *est_nframes,
  ...
  if (xtc_seek(xd, 0L, SEEK_END) != exdrOK) {
    xdrfile_close(xd);
    return exdrNR;
  }
  filesize = xtc_tell(xd);
  ...
  } else /* No easy way out. We must iterate. */
  {
    ...
    if (xtc_seek(xd, (int64_t)XTC_HEADER_SIZE, SEEK_SET) != exdrOK) {
      xdrfile_close(xd);
      return exdrNR;
    }
    ...
    *n_frames = 1;
    while (1) {
      if (xtc_seek(xd, (int64_t)(framebytes + XTC_HEADER_SIZE), SEEK_CUR) !=
          exdrOK) {
        free(*offsets);
        xdrfile_close(xd);
        return exdrNR;
      ...
      (*offsets)[*n_frames] =
          xtc_tell(xd) - 4L - (int64_t)(XTC_HEADER_SIZE); // Account for the
                                                          // header and the
                                                          // nbytes bytes we
                                                          // read.
      (*n_frames)++;
