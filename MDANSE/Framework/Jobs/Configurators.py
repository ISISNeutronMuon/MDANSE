...
class McStasOptionsConfigurator(Configurator):
    ...
    _default = {"ncount" : 10000, "dir" : os.path.join(PREFERENCES['working_directory'],"mcstas_output",time.strftime("%d.%m.%Y-%H:%M:%S", time.localtime()))}
    ...
    def configure(self, configuration, value):
        ...
        for k,v in value.items():
            if k not in options:
                LOGGER('Unknown or unsupported mcstas option: %s' % k, 'warning')
                continue
            options[k] = v
        ...
        for k,v in options.items():     
            if k == "dir":
                if os.path.exists(v):
                    LOGGER("The mcstas output directory already exists. It will be replaced by %s directory" % self._default['dir'], 'warning')
                    v =  self._default['dir']
