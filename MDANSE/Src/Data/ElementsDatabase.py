...
class ElementsDatabase(object, metaclass=Singleton):
    ...
    # The user path
    _USER_DATABASE = os.path.join(
        PLATFORM.application_directory(), "elements_database.csv"
    )
    ...
    def __getitem__(self, item):
        ...
        try:
            ...
            raise ElementsDatabaseError(
                "The element %r is not registered in the database." % ename
            )
        else:
            try:
                ...
                raise ElementsDatabaseError(
                    "The property %r is not registered in the database." % pname
                )
    ...
    def _export_csv(
        self, filename, delimiter=",", lineterminator="\n", restval="undefined"
    ):
        """
        ...
        for ename, props in list(self._data.items()):
            ...
            p.update(
                dict(
                    [
                        (pname, props.get(pname, prop()))
                        for pname, prop in list(self._properties.items())
                    ]
                )
            )
    ...
    def add_element(self, ename):
        ...
        if ename not in self._data:
            self._data[ename] = collections.OrderedDict(
                [(pname, prop()) for pname, prop in self._properties.items()]
            )
    ...
    def add_property(self, pname, typ):
        ...
        if pname in self._properties:
            raise ElementsDatabaseError(
                "The property %r is already registered in the database." % pname
            )
    ...
    def get_property(self, pname):
        ...
        if pname not in self._properties:
            raise ElementsDatabaseError(
                "The property %r is not registered in the elements database" % pname
            )
    ...
    def get_numeric_properties(self):
        ...
        return [
            name
            for name, typ in list(self._properties.items())
    ...
    def match_numeric_property(self, pname, value, tolerance=0.0):
        ...
        return [
            ename
            for ename, pval in list(pvalues.items())
            if abs(pval - value) < tolerance
        ]
