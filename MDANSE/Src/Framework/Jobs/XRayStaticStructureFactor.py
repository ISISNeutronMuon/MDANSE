...
def atomic_scattering_factor(element, qvalues):
    ...
    return c + np.sum(a[:,np.newaxis] * np.exp(-b[:,np.newaxis]*(qvalues[np.newaxis,:]/(4.0*np.pi))**2),axis=0)
...
class XRayStaticStructureFactor(DistanceHistogram):
    ...
    def finalize(self):
        ...
        for pair in self._elementsPairs:
            ...
            self._outputData['xssf_intra_%s%s' % pair][:] = fact1*np.sum((r**2)*pdfIntra*sincqr,axis=1)*dr
            self._outputData['xssf_inter_%s%s' % pair][:] = 1.0 + fact1*np.sum((r**2)*(pdfInter-1.0)*sincqr,axis=1)*dr
