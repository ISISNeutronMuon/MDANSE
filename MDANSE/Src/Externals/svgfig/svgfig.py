...
if re.search("windows", platform.system(), re.I):
    try:
        ...
        _default_directory = winreg.QueryValueEx(
            winreg.OpenKey(
...
""" + (
    ...
    def save(self, fileName=None, encoding="utf-8", compresslevel=None):
        ...
        if (
            compresslevel != None
            or re.search("\.svgz$", fileName, re.I)
            or re.search("\.gz$", fileName, re.I)
        ):
            import gzip
...
def window(
    ...
    if xlogbase != None and (ix1 <= 0.0 or ix2 <= 0.0):
        raise ValueError("x range incompatible with log scaling: (%g, %g)" % (ix1, ix2))

    if ylogbase != None and (iy1 <= 0.0 or iy2 <= 0.0):
    ...
    output.__name__ = "(%g, %g), (%g, %g) -> (%g, %g), (%g, %g)%s%s" % (
        ix1,
        ix2,
        iy1,
        iy2,
...
class Fig:
    ...
    def __init__(self, *d, **kwds):
        ...
        if len(kwds) != 0:
            raise TypeError(
                "Fig() got unexpected keyword arguments %s" % list(kwds.keys())
            )
...
class Plot:
    ...
    def __init__(self, xmin, xmax, ymin, ymax, *d, **kwds):
        ...
        if len(kwds) != 0:
            raise TypeError(
                "Plot() got unexpected keyword arguments %s" % list(kwds.keys())
            )
...
class Frame:
    ...
    def __init__(self, xmin, xmax, ymin, ymax, *d, **kwds):
        ...
        if len(kwds) != 0:
            raise TypeError(
                "Frame() got unexpected keyword arguments %s" % list(kwds.keys())
            )
...
class LineAxis(Line, Ticks):
    ...
    def interpret(self):
        if self.exclude != None and not (
            isinstance(self.exclude, (tuple, list))
            and len(self.exclude) == 2
            and isinstance(self.exclude[0], (int, float))
            and isinstance(self.exclude[1], (int, float))
        ):
