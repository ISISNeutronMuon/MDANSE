...
class JobFileGenerator:
    ...
    def __create_dependencies():
        ...
        array_of_mdanse_dependencies_string = [
            "from MDANSE.Framework.Jobs.IJob import IJob"
        ]
    ...
    def __create_test(self, parameters, test_name):
        ...
        for k, v in sorted(parameters.items()):
            if k in ["output_file", "output_files"]:
                temp = "parameters[%r] = (%r, %r)\n" % (
                    k,
                    self.reference_data_path.replace("\\", "/") + "_mono",
                    v[1],
                )
                key = k
            else:
        ...
        test_string += (
            '        reference_data_path = "'
            + self.reference_data_path.replace("\\", "/")
            + '"\n'
        )
        ...
        if self.job._type == "gmft":
            test_string += (
                '        job.configuration["trajectory"]["instance"].universe.__class__.__name__ = '
    ...
    def __generate_test_file(self, parameters, job_id):
        ...
        file_body = (
            self.__create_compare_function()
...
if __name__ == "__main__":
    ...
    for job_id, job in IJob.indirect_subclass_dictionary.items():
