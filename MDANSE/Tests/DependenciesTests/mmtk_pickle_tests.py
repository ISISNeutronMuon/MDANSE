...
class PeptideTest(unittest.TestCase):
    ...
    def test_normal_modes(self):
        ...
        for i in range(len(emodes)):
            self.assertEqual(emodes[i].force_constant, restored_modes[i].force_constant)
            err = numpy.minimum.reduce(
                numpy.fabs(numpy.ravel(emodes[i].array - restored_modes[i].array))
            )
            self.assert_(err < 1.0e-15)
...
class WaterTest(unittest.TestCase):
    ...
    def setUp(self):
        self.universe = MMTK.InfiniteUniverse(Amber99ForceField())
        self.universe.water1 = MMTK.Molecule(
            "water", position=MMTK.Vector(0.0, 0.0, 0.0)
        )
        self.universe.water2 = MMTK.Molecule(
            "water", position=MMTK.Vector(0.5, 0.0, 0.0)
        )
