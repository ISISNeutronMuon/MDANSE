...
class RigidBodyTest(unittest.TestCase):
    ...
    def test_projections(self):
        ...
        universe.m1 = MMTK.Molecule("water", position=MMTK.Vector(0.0, 0.0, 0.0))
        universe.m2 = MMTK.Molecule("water", position=MMTK.Vector(1.0, 0.0, 0.0))
        ...
        self.assertEqual(len(complement_basis), universe.degreesOfFreedom() - 11)
        for rb in rbs:
            for t in [
                ...
                self.assert_((s.projectionOf(d) - d).norm() < 1.0e-7)
                self.assert_(s.projectionComplementOf(d).norm() < 1.0e-7)

    def test_lrb(self):
        universe = MMTK.InfiniteUniverse()
        for p in [
            MMTK.Vector(0.0, 0.0, 0.0),
            MMTK.Vector(1.0, 0.0, 0.0),
            MMTK.Vector(0.0, 1.0, 1.0),
            MMTK.Vector(0.0, 1.0, 0.0),
            MMTK.Vector(1.0, 0.0, 1.0),
            MMTK.Vector(0.0, 0.0, 1.0),
            MMTK.Vector(1.0, 1.0, 0.0),
            MMTK.Vector(1.0, 1.0, 1.0),
        ]:
    ...
    def checkOrthonormality(self, subspace):
        ...
        for i in range(ndim):
            v = basis[i]
            self.assertAlmostEqual(v.dotProduct(v), 1.0, 10)
            for j in range(i + 1, ndim):
                self.assert_(abs(v.dotProduct(basis[j])) < 1.0e-10)
...
class PeptideNormalModeTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.rm = RigidMotionSubspace(self.universe, self.universe.peptide.residues())
        self.pd = PairDistanceSubspace(
            self.universe,
