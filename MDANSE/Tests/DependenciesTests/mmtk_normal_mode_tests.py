...
class WaterTest(unittest.TestCase):
    ...
    def test_energeticModes(self):
        ...
        self.assertAlmostEqual(
            f[self.universe.water.O], af[self.universe.water.O].trace()
        )
        self.assertAlmostEqual(
            f[self.universe.water.H1], af[self.universe.water.H1].trace()
        )
        self.assertAlmostEqual(
            f[self.universe.water.H2], af[self.universe.water.H2].trace()
        )
...
class PeptideTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.subspace = RigidMotionSubspace(
            self.universe, self.universe.peptide.residues()
        )

    def test_subspaceModes(self):
        vmodes = NormalModes.VibrationalModes(self.universe, subspace=self.subspace)
        freq = vmodes.frequencies[6:]
    ...
    def test_subspaceModesWithExclusion(self):
        excluded = PairDistanceSubspace(
            self.universe,
            [(self.universe.peptide[0][0].O, self.universe.peptide[0][-1].CH3)],
        )
        vmodes = NormalModes.VibrationalModes(
            self.universe, subspace=(excluded, self.subspace)
        )
        freq = vmodes.frequencies[6:]
    ...
    def test_subspaceModesWithNumericalDifferentiation(self):
        vmodes = NormalModes.VibrationalModes(
            self.universe, subspace=self.subspace, delta=0.01
        )
        freq = vmodes.frequencies[6:]
