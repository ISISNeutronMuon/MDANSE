...
class InfiniteUniverseTest:
    def setUp(self):
        ...
        self.universe.addObject(Molecule("water", position=Vector(-0.2, 0.0, 0.0)))
        self.universe.addObject(Molecule("water", position=Vector(0.2, 0.0, 0.0)))


class OrthorhombicUniverseTest:
    def setUp(self):
        self.universe = OrthorhombicPeriodicUniverse((1.0, 1.0, 0.5))
        self.universe.addObject(Molecule("water", position=Vector(-0.2, 0.0, 0.0)))
        self.universe.addObject(Molecule("water", position=Vector(0.2, 0.0, 0.0)))
...
class ParallelepipedicUniverseTest:
    def setUp(self):
        self.universe = ParallelepipedicPeriodicUniverse(
            (Vector(1.0, 0.0, 0.0), Vector(0.0, 1.0, 0.0), Vector(0.5, 0.0, 0.5))
        )
        self.universe.addObject(Molecule("water", position=Vector(-0.2, 0.0, 0.0)))
        self.universe.addObject(Molecule("water", position=Vector(0.2, 0.0, 0.0)))
...
class TrajectoryTest:
    ...
    def test_snapshot(self):
        ...
        transformation = Translation(Vector(0.0, 0.0, 0.01)) * Rotation(
            Vector(0.0, 0.0, 1.0), 1.0 * Units.deg
        )
        ...
        snapshot = SnapshotGenerator(
            self.universe, actions=[TrajectoryOutput(trajectory, ["all"], 0, None, 1)]
        )
        snapshot()
        for i in range(100):
        ...
        for i in range(101):
            ...
            max_diff = numpy.maximum.reduce(
                numpy.ravel(numpy.fabs(configuration.array - t_configuration.array))
            )
            self.assert_(max_diff < self.tolerance)
            if configuration.cell_parameters is not None:
                max_diff = numpy.maximum.reduce(
                    numpy.fabs(
                        configuration.cell_parameters - t_configuration.cell_parameters
                    )
                )
                self.assert_(max_diff < self.tolerance)
            self.universe.setConfiguration(
...
class InfiniteUniverseTestSP(
    unittest.TestCase, InfiniteUniverseTest, TrajectoryTest, SinglePrecisionTest
):
    setUp = InfiniteUniverseTest.setUp


class InfiniteUniverseTestDP(
    unittest.TestCase, InfiniteUniverseTest, TrajectoryTest, DoublePrecisionTest
):
    setUp = InfiniteUniverseTest.setUp
    tearDown = TrajectoryTest.tearDown


class OrthorhombicUniverseTestSP(
    unittest.TestCase, OrthorhombicUniverseTest, TrajectoryTest, SinglePrecisionTest
):
    setUp = OrthorhombicUniverseTest.setUp
    tearDown = TrajectoryTest.tearDown


class OrthorhombicUniverseTestDP(
    unittest.TestCase, OrthorhombicUniverseTest, TrajectoryTest, DoublePrecisionTest
):
    setUp = OrthorhombicUniverseTest.setUp
    tearDown = TrajectoryTest.tearDown


class ParallelepipedicUniverseTestSP(
    unittest.TestCase, ParallelepipedicUniverseTest, TrajectoryTest, SinglePrecisionTest
):
    setUp = ParallelepipedicUniverseTest.setUp
    tearDown = TrajectoryTest.tearDown


class ParallelepipedicUniverseTestDP(
    unittest.TestCase, ParallelepipedicUniverseTest, TrajectoryTest, DoublePrecisionTest
):
    setUp = ParallelepipedicUniverseTest.setUp
    tearDown = TrajectoryTest.tearDown
