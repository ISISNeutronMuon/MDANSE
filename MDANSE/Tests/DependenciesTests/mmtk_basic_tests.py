...
class GroupOfAtomTest(unittest.TestCase):
    ...
    def test_numberOfAtoms(self):
        self.assertEqual(self.molecule.numberOfAtoms(), self.results["numberOfAtoms"])
...
class SuperpositionTest(unittest.TestCase):
    ...
    def test_rotation_translation(self):
        for m in [MMTK.Molecule("water"), Protein("bala1")]:
            ...
            universe.rotateAroundOrigin(MMTK.Vector(0.0, 1.0, 1.0), 0.7)
            tr, rms = universe.findTransformation(ref_conf)
            self.assert_(abs(rms) < 1.0e-5)
            universe.applyTransformation(tr)
            self.assert_(universe.rmsDifference(ref_conf) < 1.0e-5)
            axis, angle = tr.rotation().axisAndAngle()
            self.assert_(abs(angle - 0.7) < 1.0e-5)
            self.assert_(
                abs(abs(numpy.cos(axis.angle(MMTK.Vector(0.0, 1.0, 1.0)))) - 1) < 1.0e-5
            )
