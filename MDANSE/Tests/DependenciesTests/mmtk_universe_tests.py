...
class PeriodicUniverseTest:
    ...
    def test_reciprocalBasisVectors(self):
        ...
        self.assertAlmostEqual(r1 * self.a, 1.0, 14)
        self.assertAlmostEqual(r2 * self.b, 1.0, 14)
        self.assertAlmostEqual(r3 * self.c, 1.0, 14)
        self.assert_(abs(r1 * self.b) < 1.0e-15)
        self.assert_(abs(r1 * self.c) < 1.0e-15)
        self.assert_(abs(r2 * self.a) < 1.0e-15)
        self.assert_(abs(r2 * self.c) < 1.0e-15)
        self.assert_(abs(r3 * self.a) < 1.0e-15)
        self.assert_(abs(r3 * self.b) < 1.0e-15)
    ...
    def test_boxCoordinates1(self):
        for i in range(500):
            p = randomPointInBox(5.0)
            pt = self.universe.realToBoxCoordinates(p)
            self.assert_(
                (self.universe.boxToRealCoordinates(pt) - p).length() < 1.0e-14
            )
            pt = self.universe.boxToRealCoordinates(p)
            self.assert_(
                (self.universe.realToBoxCoordinates(pt) - p).length() < 1.0e-14
            )
    ...
    def test_boxCoordinates2(self):
        ...
        for atom in self.universe.atomList():
            ...
            self.assert_((configuration[atom] - rb).length() < 1.0e-15)
        configuration.convertFromBoxCoordinates()
        self.assert_(
            numpy.maximum.reduce(
                numpy.ravel(
                    numpy.fabs(
                        configuration.array - self.universe.configuration().array
                    )
                )
            )
            < 1.0e-15
            ...
            for o in self.universe:
                for bu in o.bondedUnits():
                    for bond in bu.bonds:
                        l = (bond.a1.position(cconf) - bond.a2.position(cconf)).length()
                        self.assert_(l < 0.16)


class ParallelepipedicPeriodicUniverseTest(unittest.TestCase, PeriodicUniverseTest):
    def setUp(self):
        self.a = Vector(1.5, 0.3, 0.0)
        self.b = Vector(-0.1, 0.8, 0.2)
        self.c = Vector(0.0, -0.2, 1.1)
        self.universe = ParallelepipedicPeriodicUniverse((self.a, self.b, self.c))
        self.universe.addObject(Molecule("water"))


class OrthorhombicPeriodicUniverseTest(unittest.TestCase, PeriodicUniverseTest):
    def setUp(self):
        self.a = Vector(0.7, 0.0, 0.0)
        self.b = Vector(0.0, 1.0, 0.0)
        self.c = Vector(0.0, 0.0, 0.8)
