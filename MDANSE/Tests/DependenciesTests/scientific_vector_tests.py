...
class VectorTest(unittest.TestCase):
    ...
    def shouldPass(self, Vector, isVector):
        ...
        self.assertEqual(v1[-1], 3.0)
        self.assertEqual(v1.x(), 1.0)
        self.assertEqual(v1.y(), -2.0)
        self.assertEqual(v1.z(), 3.0)
        # Check arithmetic
        self.assertEqual(v1 + v2, Vector(-1.0, -1.0, 3.0))
        self.assertEqual(v1 - v2, Vector(3.0, -3.0, 3.0))
        self.assertEqual(-v1, Vector(-1.0, 2.0, -3.0))
        self.assertEqual(v1 * v2, -4.0)
        self.assertEqual(2.0 * v1, Vector(2.0, -4.0, 6.0))
        self.assertEqual(v1 / 0.5, Vector(2.0, -4.0, 6.0))
        # Check comparisons
        self.assertTrue(v1 == v1)
        self.assertFalse(v1 == v2)
        self.assertFalse(v1 == None)
        # Check methods
        self.assertAlmostEqual(v1.length(), N.sqrt(14.0), 12)
        self.assertAlmostEqual(v1.normal()[0], v1[0] / N.sqrt(14.0), 12)
        self.assertAlmostEqual(v1.normal()[1], v1[1] / N.sqrt(14.0), 12)
        self.assertAlmostEqual(v1.normal()[2], v1[2] / N.sqrt(14.0), 12)
        self.assertAlmostEqual(v1.cross(v1).length(), 0.0, 12)
        self.assertEqual(v1.cross(v2), Vector(-3.0, -6.0, -3.0))
        self.assertAlmostEqual(v1.angle(v1), 0.0, 12)
        self.assertAlmostEqual(v1.angle(v2), N.arccos(v1.normal() * v2.normal()), 12)
    ...
    def shouldFail(self, Vector, isVector):
        ...
        # Check methods
        self.assertRaises(ZeroDivisionError, Vector(0.0, 0.0, 0.0).normal)
        self.assertRaises(TypeError, v1.angle, 0.0)
        self.assertRaises(TypeError, v1.cross, 0.0)
        self.assertRaises(TypeError, v1.dyadicProduct, 0.0)


def suite():
    loader = unittest.TestLoader()
    s = unittest.TestSuite()
    s.addTest(loader.loadTestsFromTestCase(VectorTest))
    return s
