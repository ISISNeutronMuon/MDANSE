...
@overload
def build_widget(
    parent, name, value: str | float, *, initial: str | float | None, **kwargs
) -> Field[QLineEdit]: ...


@overload
def build_widget(
    parent, name, value: int, *, initial: int | None, **kwargs
) -> Field[QSpinBox]: ...


@overload
def build_widget(
    parent, name, value: bool, *, initial: bool, **kwargs
) -> Field[QCheckBox]: ...


@overload
def build_widget(
    parent, name, value: Iterable[str], *, initial: str, **kwargs
) -> Field[QComboBox]: ...
...
def get_val(widget: QWidget) -> Any:
...
def build_grid(
    *params: Param, grid_width: int = 2, connect: Callable | None = None
    ...
    for i, param in enumerate(params):
        ...
        if connect:
            param = param._replace(connect=connect)
...
class FilterDesigner(QDialog):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.config = configurator.configurable._configuration
        ...
        self.pps_configurator = fields._pps_file_widget
    ...
    def _build_filter_settings_block(self):
        settings_box = QGroupBox("Settings", self)
        self._settings_layout = QStackedLayout()
        settings_box.setLayout(self._settings_layout)
        ...
        for filter_name, current_filter in FILTER_MAP.items():
            ...
            if Filter.Flags.DIGITAL_ONLY not in flags:
                params["bound_freq"](
                    Param(
                        "Bound frequency",
            ...
            self._custom_validators(filter_name, params)

            widget = QWidget()
            layout, self.parameter_fields[filter_name] = build_grid(
                *params.values(),
                grid_width=1,
        ...
        self._settings_layout.setCurrentIndex(0)
        return settings_box

    def _custom_validators(self, filter_name: str, params: dict[str, Param]):

        traj = self.configurator.configurable.settings["trajectory"][1]["configurator"]
        current_filter = FILTER_MAP[filter_name]
        n_steps = traj.get("n_steps", DEFAULT_N_STEPS)
        time_step = traj.get("time_step_ps", DEFAULT_TIME_STEP)
        flags = current_filter.flags
        units = (
        ...
        bin_width = round(
            Filter.frequency_resolution(n_steps, time_step, units=units),
            FLOAT_SPINBOX_DECIMALS,
        )
        vmax = round(
            Filter.nyquist(time_step, units=units) - bin_width,
            FLOAT_SPINBOX_DECIMALS,
        )
        ...
        for name in ("cutoff_freq", "fundamental_freq"):
            if param := params.get(name):
                if Filter.Flags.FUNDAMENTAL_EVENLY_DIVIDES_FS in flags:
                    ...
                    def valid_func(x):
                        return ((1 / time_step) % x) == 0
                else:
                    valid_func = None

                init = 1 / time_step * n_steps
    ...
    def settings(self) -> FilterDesSettings:
        ...
        return {
            ...
            "attributes": {
                ...
                "time_step_ps": traj["md_time_step"],
            },
        }
    ...
    def get_attenuation_function(
        ...
        match source:
            ...
            # case "Gaussian noise" | "Uniform noise":
            #     values = attenuation_fn(freqs)
            case _:
                raise ValueError(f"Unknown attenuation functional form ({source}).")
        normalised = values / np.max(values)

        if self.current_filter_units() is Filter.FrequencyUnits.CYCLIC:
            freqs /= 2 * np.pi
    ...
    def render_graph(
        ...
        *,
        ...
        axes.set_xlim(0.0, freqs.frequencies.max())
    ...
    def apply(self) -> None:
        ...
        self.configurator.configure(self.settings["parameters"])
        self.fields.set_from_configurator()
        self.close()
...
class TrajectoryFilterWidget(WidgetBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._pps_file_widget = first_true(
        ...
        self._helper_button = QPushButton(self._push_button_text, self._base)
        self._helper_button.clicked.connect(self.helper_dialog)

        self._layout.addWidget(self._helper_button, 0, 0)
        self._layout.addWidget(self._type_label, 0, 1)
        self._layout.addWidget(self._type_combo, 0, 2)
        self._layout.addWidget(self._settings_box, 1, 0, 4, 2)

        self._type_combo.setCurrentText("Butterworth")
        self.set_from_configurator()
        self.update_labels()
        self.updateValue()

    def _build_filter_settings_block(self):
        settings_box = QGroupBox("Settings", self._base)
        settings_box.setSizePolicy(
            QSizePolicy.Policy.Expanding,
            QSizePolicy.Policy.Maximum,
        )
        ...
        for filter_name, current_filter in FILTER_MAP.items():
            ...
            widget = QWidget()
            layout, self._fields[filter_name] = build_grid(
                *params,
                grid_width=3,
            )
            widget.setLayout(layout)
    ...
    def current_fields(self) -> dict[str, Field]:
        return self._fields[self.current_filter_name]
    ...
    def get_widget_value(self) -> str:
        ...
        return json.dumps(
            {
                label.text(): get_val(field)
                for label, field in self.current_fields.values()
            }
            | {"filter": get_val(self.type_combo)}
        )
