    ...
    def update(self, init=False):
        ...
        for job in jobs:
            ...
            # The job file could be opened and unpickled properly
                ...
                if running:
                    jobStartingTime = datetime.datetime.strptime(
                        info["start"], "%d-%m-%Y %H:%M:%S"
                    )
                    procStartingTime = datetime.datetime.strptime(
                        pids[info["pid"]], "%d-%m-%Y %H:%M:%S"
                    )
...
class JobControllerPanel(wx.ScrolledWindow):
    ...
    def on_display_info(self, event):
        ...
        self._info = wx.TextCtrl(
            panel, wx.ID_ANY, style=wx.TE_AUTO_SCROLL | wx.TE_READONLY | wx.TE_MULTILINE
        )
    ...
    def on_kill_job(self, event):
        ...
        d = wx.MessageDialog(
            None,
            "Do you really want to kill job %r ?" % name,
            "Question",
    ...
    def on_update(self, event):
        ...
        for jobName, jobStatus in list(runningJobs.items()):
            if jobName in self._jobs:
                ...
                self._jobs[jobName] = {}
                self._jobs[jobName]["name"] = wx.Button(
                    self, wx.ID_ANY, style=wx.BU_EXACTFIT, label=jobStatus["name"]
                )
                self._jobs[jobName]["pid"] = intctrl.IntCtrl(
                    self,
                    wx.ID_ANY,
                    style=wx.TE_READONLY | wx.ALIGN_CENTER_HORIZONTAL,
                    value=jobStatus["pid"],
                )
                self._jobs[jobName]["start"] = wx.TextCtrl(
                    self,
                    wx.ID_ANY,
                    style=wx.TE_READONLY | wx.ALIGN_CENTER_HORIZONTAL,
                    value=jobStatus["start"],
                )
                self._jobs[jobName]["elapsed"] = wx.TextCtrl(
                    self,
                    wx.ID_ANY,
                    style=wx.TE_READONLY | wx.ALIGN_CENTER_HORIZONTAL,
                    value=jobStatus["elapsed"],
                )
                self._jobs[jobName]["state"] = wx.Button(
                    self, wx.ID_ANY, style=wx.BU_EXACTFIT, label=jobStatus["state"]
                )
                self._jobs[jobName]["progress"] = wx.Gauge(self, wx.ID_ANY, range=100)
                self._jobs[jobName]["progress"].SetValue(jobStatus["progress"])
                self._jobs[jobName]["eta"] = wx.TextCtrl(
                    self,
                    wx.ID_ANY,
                    style=wx.TE_READONLY | wx.ALIGN_CENTER_HORIZONTAL,
                    value=jobStatus["eta"],
                )
                self._jobs[jobName]["kill"] = wx.BitmapButton(
                    self, wx.ID_ANY, ICONS["stop", 16, 16]
                )

                r = len(self._jobs)

                self._gbSizer.Add(
                    self._jobs[jobName]["name"],
                    pos=(r, 0),
                    flag=wx.EXPAND | wx.ALIGN_CENTER_HORIZONTAL,
                )
                self._gbSizer.Add(
                    self._jobs[jobName]["pid"],
                    pos=(r, 1),
                    flag=wx.EXPAND | wx.ALIGN_CENTER_HORIZONTAL,
                )
                self._gbSizer.Add(
                    self._jobs[jobName]["start"],
                    pos=(r, 2),
                    flag=wx.EXPAND | wx.ALIGN_CENTER_HORIZONTAL,
                )
                self._gbSizer.Add(
                    self._jobs[jobName]["elapsed"],
                    pos=(r, 3),
                    flag=wx.EXPAND | wx.ALIGN_CENTER_HORIZONTAL,
                )
                self._gbSizer.Add(
                    self._jobs[jobName]["state"],
                    pos=(r, 4),
                    flag=wx.EXPAND | wx.ALIGN_CENTER_HORIZONTAL,
                )
                self._gbSizer.Add(
                    self._jobs[jobName]["progress"],
                    pos=(r, 5),
                    flag=wx.EXPAND | wx.ALIGN_CENTER_HORIZONTAL,
                )
                self._gbSizer.Add(
                    self._jobs[jobName]["eta"],
                    pos=(r, 6),
                    flag=wx.EXPAND | wx.ALIGN_CENTER_HORIZONTAL,
                )
                self._gbSizer.Add(
                    self._jobs[jobName]["kill"],
                    pos=(r, 7),
                    flag=wx.EXPAND | wx.ALIGN_CENTER_HORIZONTAL,
                )
            ...
            self.Bind(
                wx.EVT_BUTTON, self.on_display_traceback, self._jobs[jobName]["state"]
            )
