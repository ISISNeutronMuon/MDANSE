...
class IPlugin(wx.Panel):
    ...
    def on_close_pane(self, event):
        d = wx.MessageDialog(
            None,
            "Closing this plugin will also close all the other ones you plugged in in so far. Do you really want to close ?",
            "Question",
    ...
    def drop(self, pluginName):
        ...
        self._mgr.AddPane(
            plugin, aui.AuiPaneInfo().Caption(getattr(plugin, "label", pluginName))
        )
