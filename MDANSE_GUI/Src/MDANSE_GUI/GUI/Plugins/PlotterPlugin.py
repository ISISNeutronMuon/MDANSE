...
class DataPanel(wx.Panel):
    ...
    def build_panel(self):
        ...
        if self.standalone:
            ...
            self.datasetlist = wx.ListCtrl(
                splitterWindow, wx.ID_ANY, style=wx.LC_REPORT | wx.LC_SINGLE_SEL
            )
            ...
            self.Bind(
                wx.EVT_LIST_ITEM_SELECTED, self.on_select_dataset, self.datasetlist
            )
            self.Bind(wx.EVT_LIST_KEY_DOWN, self.delete_dataset, self.datasetlist)

            self.datalist = wx.ListCtrl(
                splitterWindow, wx.ID_ANY, style=wx.LC_REPORT | wx.LC_SINGLE_SEL
            )
        else:
            self.datalist = wx.ListCtrl(
                self.setup, wx.ID_ANY, style=wx.LC_REPORT | wx.LC_SINGLE_SEL
            )
        ...
        self._selectedDimension = wx.SpinCtrl(
            self.setup, id=wx.ID_ANY, style=wx.SP_WRAP | wx.SP_ARROW_KEYS
        )
        self._selectedDimension.SetRange(0, 2)
        sliceText = wx.StaticText(self.setup, label="Slice:")
        self._selectedSlice = wx.SpinCtrl(
            self.setup, id=wx.ID_ANY, style=wx.SP_WRAP | wx.SP_ARROW_KEYS
        )
        ...
        self.replot_button = wx.Button(
            self.setup, wx.ID_ANY, label="Plot in current figure"
        )
    ...
    def on_plot(self, event=None):
        ...
        if plot_type == "Line":
            ...
            self.plotterNotebook.AddPage(
                Plotter, "%s(%s)" % (self.selectedVar, plot_type)
            )
    ...
    def on_replot(self, event=None):
        ...
        if plot_type == "Image" and self.selectedPlot.type == "image":
            ...
            if hasattr(self.selectedPlot, "AddPane"):
                ...
                Plotter.plot(data, self.selectedVar)
                self.selectedPlot.AddPane(
                    Plotter,
                    wxaui.AuiPaneInfo()
                    .Right()
                    .CloseButton(True)
                    .CaptionVisible(True)
                    .Caption(self.selectedVar)
                    .MinSize((200, -1)),
                )
                ...
                multiplot.AddPane(
                    ...
                    .CloseButton(True)
                    .CaptionVisible(True)
                    .Caption(self.selectedPlot.varname),
                )
                multiplot.Update()
                ...
                self.plotterNotebook.AddPage(
                    multiplot, "*MultiView*(Image)", select=True
                )
    ...
    def on_load_data(self, event=None):
        ...
        for basename, filename in zip(baselist, filelist):
            ...
            with PLOTTER_DATA_TYPES[ext](filename, "r") as f:
                ...
                self.plugin._dataDict[unique_name] = {
                    "data": data,
                    "path": filename,
                    "basename": basename,
                }
