...
class QVectorsPlugin(UserDefinitionPlugin):
    ...
    def build_panel(self):
        ...
        self._notebook = wxaui.AuiNotebook(
            self._mainPanel,
            wx.ID_ANY,
            style=wxaui.AUI_NB_DEFAULT_STYLE & ~wxaui.AUI_NB_TAB_MOVE,
        )
    ...
    def set_trajectory(self, trajectory):
        ...
        if self._trajectory.universe.is_periodic:
            ...
            choices = [
                k for k, v in list(REGISTRY["q_vectors"].items()) if not v.is_lattice
            ]
    ...
    def on_generate_q_vectors(self, event):
        ...
        startWorker(
            self.generate_q_vectors_done,
            self.generate_q_vectors,
            cargs=(qPanel,),
            wargs=(generator,),
        )
    ...
    def select_generator(self, generatorName):
        ...
        self._currentGenerator = REGISTRY["q_vectors"][generatorName](
            self._trajectory.universe
        )
        ...
        self._configurationPanel = ConfigurationPanel(
            self._mainPanel, self._currentGenerator, None
        )
