...
from MDANSE.Framework.AtomSelectionParser import (
    AtomSelectionParser,
...
class AtomSelectionPlugin(UserDefinitionPlugin):
    ...
    def __init__(self, parent, *args, **kwargs):
        ...
        PUBLISHER.subscribe(
            self.msg_select_atoms_from_viewer, "msg_select_atoms_from_viewer"
        )
    ...
    def build_panel(self):
        ...
        self.values = wx.ListBox(
            self._queryPanel, wx.ID_ANY, style=wx.LB_MULTIPLE | wx.LB_NEEDED_SB
        )
        ...
        keywordsValuesSizer.Add(
            self.filterTree,
            1,
            wx.ALIGN_CENTER_VERTICAL | wx.LEFT | wx.RIGHT | wx.EXPAND,
            2,
        )
        keywordsValuesSizer.Add(
            self.values, 3, wx.ALIGN_CENTER_VERTICAL | wx.LEFT | wx.RIGHT | wx.EXPAND, 2
        )
        ...
        selectionExpressionStaticBox = wx.StaticBox(
            self._mainPanel, wx.ID_ANY, label="Selection"
        )
        selectionExpressionStaticBoxSizer = wx.StaticBoxSizer(
            selectionExpressionStaticBox, wx.HORIZONTAL
        )

        self.selectionTextCtrl = wx.TextCtrl(
            self._mainPanel, wx.ID_ANY, style=wx.TE_READONLY
        )
        ...
        self._selectionExpressionSizer.Add(
            self.selectionTextCtrl, pos=(0, 0), span=(1, 2), flag=wx.ALL | wx.EXPAND
        )
        self._selectionExpressionSizer.Add(clearButton, pos=(0, 2), flag=wx.ALL)
        selectionExpressionStaticBoxSizer.Add(
            self._selectionExpressionSizer, 1, wx.ALL | wx.EXPAND, 5
        )

        self._selectionSummary = wx.TextCtrl(
            self._mainPanel,
            wx.ID_ANY,
        ...
        self.Bind(
            wx.EVT_TREE_SEL_CHANGED, self.on_display_keyword_values, self.filterTree
        )
    ...
    def plug(self):
        self._parent._mgr.GetPane(self).Float().Floatable(True).Dockable(
            True
        ).CloseButton(True)
    ...
    def on_clear(self, event=None):
        ...
        PUBLISHER.sendMessage("msg_clear_selection", message=self)
    ...
    def on_display_keyword_values(self, event=None):
        ...
        if selectionFilter not in self._selectors:
            self._selectors[selectionFilter] = [
                str(v)
                for v in REGISTRY["selector"][selectionFilter](self._trajectory).choices
            ]
    ...
    def set_selection(self):
        ...
        PUBLISHER.sendMessage("msg_set_selection", message=self)
