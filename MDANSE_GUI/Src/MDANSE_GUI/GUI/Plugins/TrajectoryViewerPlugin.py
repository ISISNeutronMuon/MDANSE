...
class TrajectoryViewerPlugin(ComponentPlugin):
    ...
    def build_panel(self):
        ...
        self._selectedAtom = wx.SpinCtrl(
            panel, wx.ID_ANY, style=wx.SP_WRAP | wx.SP_ARROW_KEYS
        )

        selectedDimensionText = wx.StaticText(panel, wx.ID_ANY, label="Dimension:")
        self._selectedDimension = wx.ComboBox(
            panel, wx.ID_ANY, choices=["x", "y", "z"], style=wx.CB_READONLY
        )

        selectedOptionsSizer.Add(
            selectedVariableText, 0, wx.LEFT | wx.ALIGN_CENTER_VERTICAL, 2
        )
        selectedOptionsSizer.Add(self._selectedVariable, 1, wx.CENTER | wx.EXPAND, 2)
        selectedOptionsSizer.Add(
            selectedAtomText, 0, wx.LEFT | wx.ALIGN_CENTER_VERTICAL, 2
        )
        selectedOptionsSizer.Add(self._selectedAtom, 1, wx.CENTER | wx.EXPAND, 2)
        selectedOptionsSizer.Add(
            selectedDimensionText, 0, wx.LEFT | wx.ALIGN_CENTER_VERTICAL, 2
        )
    ...
    def _plot(self, variable, atom, dimension):
        dimensionNumber = TrajectoryViewerPlugin._dimensions[
            self._selectedDimension.GetValue()
        ]

        time = self._trajectory.file["time"]
        data = self._trajectory.read_configuration_trajectory(
            atom, 0, len(self._trajectory), 1, variable=variable
        )
        ...
        self._figure.gca().set_ylabel(
            "%s (%s)" % (variable, TrajectoryViewerPlugin._units.get(variable, "au"))
        )
        self._figure.gca().plot(
            time[:], data[:, dimensionNumber], label=label, picker=3
        )
        self._canvas.draw()
