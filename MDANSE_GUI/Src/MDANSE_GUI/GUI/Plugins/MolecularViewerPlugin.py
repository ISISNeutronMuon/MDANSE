...
class MolecularViewerPlugin(ComponentPlugin):
    ...
    def build_panel(self):
        ...
        self._mgr.AddPane(
            self._iren,
            aui.AuiPaneInfo().Dock().Center().CaptionVisible(False).CloseButton(False),
        )
    ...
    def plug(self):
        self._parent._mgr.GetPane(self).Dock().Floatable(False).Center().CloseButton(
            True
        )
    ...
    def set_trajectory(self, trajectory, selection=None, frame=0):
        ...
        self._atomsScales = np.array(
            [ATOMS_DATABASE[at.symbol]["vdw_radius"] for at in self._atoms]
        ).astype(np.float32)

        scalars = ndarray_to_vtkarray(
            self.atomsColours, self._atomsScales, self._nAtoms
        )
    ...
    def color_string_to_RGB(self, s):
        ...
        return np.array(s.split(";")).astype(np.float32) / 255.0
    ...
    def build_ColorTransferFunction(self):
        ...
        color_string_list = [
            self.color_string_to_RGB(ATOMS_DATABASE[at.symbol]["color"])
            for at in self._atoms
        ]
    ...
    def enable_picking(self, state):
        if state:
            self.__pickerObserverId = self._iren.AddObserver(
                "LeftButtonPressEvent", self.on_pick
            )
        else:
    ...
    def on_show_popup_menu(self, event):
        ...
        selectionBox = popupMenu.Append(
            wx.ID_ANY, "Show/hide selection box", kind=wx.ITEM_CHECK
        )
        ...
        parallelProjection = popupMenu.Append(
            wx.ID_ANY, "Parallel projection", kind=wx.ITEM_CHECK
        )
        ...
        boundingBox = popupMenu.Append(
            wx.ID_ANY, "Show/hide bounding box", kind=wx.ITEM_CHECK
        )
    ...
    def on_info_pick(self, obj, evt=None):
        ...
        if pid > 0:
            ...
            info = "%s (id:%s) at  %s" % (
                self._atoms[idx].full_name(),
                self._atoms[idx].index,
                "%.3f %.3f %.3f" % tuple(xyz),
            )
            LOGGER(info, "info")
    ...
    def pick_atoms(self, atomsList, new=False):
        ...
        PUBLISHER.sendMessage(
            ...
            message=(self.dataplugin, list(self.__pickedAtoms)),
        )
    ...
    def on_clear_selection(self, event=None):
        ...
        for i in range(self._nAtoms):
            self._polydata.GetPointData().GetArray("scalars").SetTuple3(
                i, self._atomsScales[i], self._atomsColours[i], i
            )
    ...
    def on_save_selection(self, event=None):
        ...
        UD_STORE.set_definition(
            target, "atom_selection", name, {"indexes": sorted(self.__pickedAtoms)}
        )
    ...
    def show_selection(self, selection):
        ...
        for idx in range(self._nAtoms):
            self._polydata.GetPointData().GetArray("scalars").SetTuple3(
                idx, self._atomsScales[idx], self.atomsColours[idx], idx
            )
    ...
    def get_atom_index(self, pid):
        if self._rendmod in [1, 3, 4]:
            ...
            _, _, idx = (
                self.tubes.GetOutput().GetPointData().GetArray("scalars").GetTuple3(pid)
            )
        else:
    ...
    def get_atom_props(self, pid):
        if self._rendmod in [1, 3, 4]:
            ...
            radius, color, idx = (
                self._polydata.GetPointData().GetArray("scalars").GetTuple3(pid)
            )
    ...
    def pick_atom(self, pid):
        if self.this_atom_is_selected(pid):
            ...
            self._polydata.GetPointData().GetArray("scalars").SetTuple3(
                pid, radius, RGB_COLOURS["selection"][0], pid
            )
            self._polydata.Modified()
    ...
    def unpick_atom(self, pid):
        ...
        self._polydata.GetPointData().GetArray("scalars").SetTuple3(
            pid, radius, self._atomsColours[pid], pid
        )
        self._polydata.Modified()
    ...
    def build_bbox(self, basis_vector):
        ...
        Xmax, Ymax, Zmax = np.array(
            [cell[0, 0], cell[1, 1], cell[2, 2]], dtype=np.float32
        )
        bbox_points = vtk.vtkPoints()

        self.cell_bounds_coords = np.array(
            [-Xmax / 2.0, Xmax / 2.0, -Ymax / 2.0, Ymax / 2.0, -Zmax / 2.0, Zmax / 2.0]
        )

        bbox_points.InsertNextPoint(-Xmax / 2.0, -Ymax / 2.0, -Zmax / 2.0)
        bbox_points.InsertNextPoint(Xmax / 2.0, Ymax / 2.0, Zmax / 2.0)
    ...
    def build_scene(self):
        ...
        self.picking_domain = [lineActor, ballActor, tubeActor, ballActor, ballActor][
            rendmod
        ]
    ...
    def set_configuration(self, frame):
        ...
        if unitCell is None:
            ...
            conf = PeriodicRealConfiguration(
                self.trajectory.chemical_system, coords, unitCell
            )
...
def get_trajectory_filename():
    ...
    dialog = wx.FileDialog(
        None, message="Open Trajectory file...", wildcard=filters, style=wx.FD_OPEN
    )
