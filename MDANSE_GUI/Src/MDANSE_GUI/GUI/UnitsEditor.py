...
class FloatValidator(wx.PyValidator):
    ...
    def Validate(self, win):
        ...
        try:
            ...
            wx.MessageBox(
                "Please enter numbers only", "Invalid Input", wx.OK | wx.ICON_ERROR
            )
            return False
        else:
            return True
...
class NewUnitDialog(wx.Dialog):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for comp, k in enumerate(self._dimensions):
            dimensionsSizer.Add(
                self._labels[k], (comp, 0), flag=wx.ALIGN_CENTER_VERTICAL
            )
    ...
    def on_ok(self, event):
        ...
        if not unit:
            wx.MessageBox(
                "Please enter a unit name", "Invalid Input", wx.OK | wx.ICON_ERROR
            )
            return
        ...
        try:
            ...
            wx.MessageBox(
                "Please enter a valid number for factor",
                "Invalid Input",
                wx.OK | wx.ICON_ERROR,
            )
            return
        ...
        if UNITS_MANAGER.has_unit(unit):
            wx.MessageBox(
                "This unit already exists", "Invalid Input", wx.OK | wx.ICON_ERROR
            )
            return
...
class UnitsEditor(wx.Dialog):
    ...
    def _build(self):
        ...
        self._unitsList = wx.ListBox(
            mainPanel,
            wx.ID_ANY,
            choices=sorted(self._defaultUnits.keys()),
        ...
        for uname in _UNAMES:
            ...
            self._dimensions[uname] = wx.TextCtrl(
                dimensionsPanel, id=wx.ID_ANY, style=wx.TE_READONLY
            )
    ...
    def on_add_unit(self, event):
        ...
        if UNITS_MANAGER.has_unit(unitName):
            d = wx.MessageDialog(
                None,
                "This unit already exists. Do you want to overwrite it ?",
                "Question",
    ...
    def on_ok(self, event, save=False):
        if save:
            ...
            if self._standalone:
                wx.MessageBox(
                    "Units database saved successfully",
                    "Success",
