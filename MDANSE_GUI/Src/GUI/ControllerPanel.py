...
class ControllerPanel(wx.Panel):
    ...
    def build_panel(self):
        self._notebook = wxaui.AuiNotebook(
            self, style=wxaui.AUI_NB_DEFAULT_STYLE & ~wxaui.AUI_NB_CLOSE_ON_ACTIVE_TAB
        )

        self._pages = collections.OrderedDict()
        self._pages["logger"] = wx.TextCtrl(
            self,
        ...
        if wx.VERSION[:2] > (2, 8):
            ...
            self._notebook.AddPage(
                self._pages["logger"], "Logger", bitmap=ICONS["log", 16, 16]
            )
            self._notebook.AddPage(
                self._pages["console"], "Console", bitmap=ICONS["shell", 16, 16]
            )
            self._notebook.AddPage(
                self._pages["jobs"], "Jobs", bitmap=ICONS["hourglass", 16, 16]
            )
