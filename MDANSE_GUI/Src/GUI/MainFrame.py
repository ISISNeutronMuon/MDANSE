...
class MainFrame(wx.Frame):
    def __init__(
        self,
        parent,
        title="MDANSE: Molecular Dynamics Analysis for Neutron Scattering Experiments",
    ):
        ...
        consoleHandler = REGISTRY["handler"]["console"](
            self._panels["controller"].pages["logger"]
        )
    ...
    def build_dialog(self):
        ...
        self._mgr.AddPane(
            self._panels["working"],
            paneInfo3.Caption("Working panel")
            .Name("working")
            .Center()
            .CloseButton(False),
        )
        ...
        self._mgr.AddPane(
            self._panels["controller"],
            paneInfo4.Name("controller")
            .Name("controller")
            .Floatable()
            .Right()
            .Bottom()
    ...
    def build_menu(self):
        ...
        converters = [
            job
            for job in list(REGISTRY["job"].items())
            if issubclass(job[1], Converter)
        ]
    ...
    def build_toolbar(self):
        ...
        loadDataButton = self._toolbar.AddTool(
            wx.ID_ANY, "Load a trajectory", ICONS["load", 32, 32]
        )
        periodicTableButton = self._toolbar.AddTool(
            wx.ID_ANY,
            "Launch the periodic table viewer",
            ICONS["periodic_table", 32, 32],
        )
        elementsDatabaseButton = self._toolbar.AddTool(
            wx.ID_ANY, "Launch the elements database editor", ICONS["atom", 32, 32]
        )
        plotButton = self._toolbar.AddTool(
            wx.ID_ANY, "Launch the NetCDF plotter", ICONS["plot", 32, 32]
        )
        udButton = self._toolbar.AddTool(
            wx.ID_ANY, "Launch the user definitions editor", ICONS["user", 32, 32]
        )
        unitsButton = self._toolbar.AddTool(
            wx.ID_ANY, "Launch the units editor", ICONS["units", 32, 32]
        )
        registryButton = self._toolbar.AddTool(
            wx.ID_ANY, "Inspect MDANSE classes framework", ICONS["registry", 32, 32]
        )
        templateButton = self._toolbar.AddTool(
            wx.ID_ANY, "Save a template for a new analysis", ICONS["template", 32, 32]
        )
        apiButton = self._toolbar.AddTool(
            wx.ID_ANY, "Open MDANSE API", ICONS["api", 32, 32]
        )
        websiteButton = self._toolbar.AddTool(
            wx.ID_ANY, "Open MDANSE website", ICONS["web", 32, 32]
        )
        aboutButton = self._toolbar.AddTool(
            wx.ID_ANY, "About MDANSE", ICONS["about", 32, 32]
        )
        bugButton = self._toolbar.AddTool(wx.ID_ANY, "Bug report", ICONS["bug", 32, 32])
        quitButton = self._toolbar.AddTool(
            wx.ID_ANY, "Quit MDANSE", ICONS["quit", 32, 32]
        )
...
""" % (
        ...
        d = wx.MessageDialog(
            self, about_str, "About", style=wx.OK | wx.ICON_INFORMATION
        )
        d.ShowModal()
        d.Destroy()
    ...
    def on_simple_help(self, event):
        ...
        with open(path, "r") as f:
            ...
            html = wxhtml.HtmlWindow(
                panel,
                -1,
                size=(300, 150),
                style=wx.VSCROLL | wx.HSCROLL | wx.TE_READONLY | wx.BORDER_SIMPLE,
            )
            html.SetPage(info)
    ...
    def on_theory_help(event):
        webbrowser.open(
            "file://%s" % os.path.join(PLATFORM.doc_path(), "theory_help.pdf")
        )
    ...
    def on_load_data(self, event=None):
        wildcards = collections.OrderedDict(
            [kls._type, "%s (*.%s)|*.%s" % (kls._type, kls.extension, kls.extension)]
            for kls in list(REGISTRY["input_data"].values())
            if kls.extension is not None
        )

        dialog = wx.FileDialog(
            None,
            message="Open data ...",
            wildcard="automatic (*.mvi,*.nc,*.h5)|*.mvi;*.nc;*.h5|"
            + "|".join(wildcards.values()),
            style=wx.FD_OPEN,
        )
