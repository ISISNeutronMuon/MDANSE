...
class AtomNameDropTarget(wx.TextDropTarget):
    ...
    def OnDropText(self, x, y, data):
        listedAtoms = [
            self._atoms.GetItem(idx).GetText()
            for idx in range(self._atoms.GetItemCount())
        ]
        ...
        if self._atoms.GetItemCount() > 0:
            if molname != self._currentMolecule:
                LOGGER(
                    "The dragged atoms does not belong to the same molecule than the current selection.",
                    "error",
                    ["dialog"],
                )
                return
...
class AtomsListPlugin(UserDefinitionPlugin):
    ...
    def build_panel(self):
        ...
        self._nAtomsSpinCtrl = wx.SpinCtrl(
            self._mainPanel, wx.ID_ANY, style=wx.SP_ARROW_KEYS | wx.SP_WRAP
        )
        ...
        self._resultsTextCtrl = wx.TextCtrl(
            self._mainPanel, wx.ID_ANY, style=wx.TE_READONLY | wx.TE_MULTILINE
        )
    ...
    def plug(self):
        self.parent.mgr.GetPane(self).Float().Dockable(False).CloseButton(
            True
        ).BestSize((600, 600))
    ...
    def on_add_atom(self, event):
        ...
        if self._atoms.GetItemCount() == self._nAtoms:
            ...
            self._selection = find_atoms_in_molecule(
                self._trajectory.universe, molecule, self._selectedAtoms, True
            )
            ...
            for idxs in self._selection:
                line = "  ;  ".join(
                    [
                        "Atom %5d : %s" % (v, self._selectedAtoms[i])
