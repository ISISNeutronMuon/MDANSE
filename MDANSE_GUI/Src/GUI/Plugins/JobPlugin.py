...
class JobPlugin(ComponentPlugin):
    ...
    def build_panel(self):
        ...
        self._main.SetScrollbars(
            pixelsPerUnitX=20, pixelsPerUnitY=20, noUnitsX=500, noUnitsY=500
        )
    ...
    def plug(self):
        self._parent._mgr.GetPane(self).Float().Center().Floatable(True).Dockable(
            True
        ).CloseButton(True)
...
if __name__ == "__main__":
    filename = os.path.join(
        os.path.dirname(PLATFORM.package_directory()),
        "Data",
        "Trajectories",
        "HDF",
        "waterbox.h5",
