...
class CrossSlicer(object):
    def __init__(
        self, parent, canvas, dialog, hplot, vplot, fig, color_index, hlegend, vlegend
    ):
...
class Plotter2D(wx.Panel):
    ...
    def __init__(self, parent, data=None):
        ...
        self.annotation = wx.StaticText(
            self, label="x : , y : , data[x,y] :", style=wx.ALIGN_CENTER_VERTICAL
        )

        hsizer = wx.BoxSizer()
        hsizer.Add(self.toolbar, 0, flag=wx.ALL | wx.EXPAND)
        hsizer.Add(
            self.slice_checkbox, 0, flag=wx.EXPAND | wx.ALIGN_CENTER_VERTICAL | wx.ALL
        )
        ...
        self.color_list = collections.OrderedDict(
            [
                ["blue", "b"],
                ["green", "g"],
                ["red", "r"],
            ...
            if self.Xmin <= x and x <= self.Xmax and self.Ymin <= y and y <= self.Ymax:
                try:
                    ...
                    self.annotation.SetLabel(
                        "x : %g (%g), y : %g (%g), data[x,y] : %g"
                        % (
                            X,
                            x * self.Xunit_conversion_factor,
                            Y,
                            y * self.Yunit_conversion_factor,
                            i,
                        )
                    )
                except:
    ...
    def export_data(self, event=None):
        ...
        data = np.vstack(
            (
                ...
                np.hstack(
                    (
                        self.Yaxis[:, np.newaxis] * self.Yunit_conversion_factor,
                        self.data,
                    )
                ),
            )
    ...
    def slice(self, event):
        ...
        self.slice_event_id = self.canvas.mpl_connect(
            "button_press_event", self.build_slice_widget
        )
    ...
    def build_slice_widget(self, event):
        if not event.inaxes:
            ...
            if self.cross_slice_dialog is None or not self.cross_slice_dialog:
                ...
                self.cross_slice_canvas = FigureCanvasWxAgg(
                    self.cross_slice_dialog, wx.ID_ANY, self.cross_slice_fig
                )
                self.cross_slice_toolbar = NavigationToolbar2WxAgg(
                    self.cross_slice_canvas
                )

                self.unique_target_checkbox = wx.CheckBox(
                    self.cross_slice_dialog, id=wx.ID_ANY, label="Single target plot"
                )
                self.Bind(
                    wx.EVT_CHECKBOX,
                    self.made_unique_target,
                    id=self.unique_target_checkbox.GetId(),
                )
                self.unique_target_checkbox.SetValue(self.unique)

                self.auto_scale_button = wx.Button(
                    self.cross_slice_dialog, id=wx.ID_ANY, label="Auto-Scale"
                )
                self.Bind(
                    wx.EVT_BUTTON, self.auto_scale_cross_plot, self.auto_scale_button
                )
            ...
            self.slice_widget.append(
                self.subplot.axvline(
                    x=x,
                    linewidth=3,
                    color="%s" % self.get_circular_color(self.cross_slice_color_index),
                )
            )
            self.slice_widget.append(
            ...
            v_tmp_plot = self.v_cross_slice_plot.plot(
                self.Xaxis * self.Xunit_conversion_factor,
                vslice,
                color="%s" % self.get_circular_color(self.cross_slice_color_index),
            )
            h_tmp_plot = self.h_cross_slice_plot.plot(
                self.Yaxis * self.Yunit_conversion_factor,
                hslice,
                color="%s" % self.get_circular_color(self.cross_slice_color_index),
            )

            self.v_cross_slice_legend.append(
                [v_tmp_plot[0], "%s = %8.3f" % (self.Ylabel, y)]
            )
            self.h_cross_slice_legend.append(
                [h_tmp_plot[0], "%s = %8.3f" % (self.Xlabel, x)]
            )
            ...
            self.v_cross_slice_plot.legend(
            ...
            self.h_cross_slice_plot.legend(
                tuple([e[0] for e in self.h_cross_slice_legend]),
                tuple([e[1] for e in self.h_cross_slice_legend]),
                loc="best",
                frameon=True,
                shadow=True,
            )
    ...
    def made_unique_target(self, event):
        self.parent.unique_slicer = CrossSlicer(
            self,
            self.cross_slice_canvas,
    ...
    def plot(
        ...
        self.ax = self.subplot.imshow(
            self.data, interpolation=self.interpolation, origin="lower"
        )
    ...
    def scale(self, onReplot=False):
        ...
        try:
            ...
            raise Plotter2DError(
                "Could not set normalization : difference between minimum and maximum values is to small"
            )
    ...
    def set_axis_property(self, varname, data):
        ...
        if not oldXunit is None:
            if oldXunit != self.Xinit_unit:
                raise Plotter2DError(
                    "the x axis unit (%s) of data-set %r is inconsistent with the unit (%s) of the precedent data plotted "
                    % (self.Xinit_unit, varname, oldXunit)
                )
        if not oldYunit is None:
            if oldYunit != self.Yinit_unit:
                raise Plotter2DError(
                    "the y axis unit (%s) of data-set %r is inconsistent with the unit (%s) of the precedent data plotted "
                    % (self.Yinit_unit, varname, oldYunit)
                )
