...
class Plotter3D(wx.Panel):
    ...
    def build_panel(self):
        ...
        self.iren = wxVTKRenderWindowInteractor(
            self.viewer, -1, size=(500, 500), flag=wx.EXPAND
        )
    ...
    def array_to_2d_imagedata(self):
        ...
        vtk_array = numpy_to_vtk(
            num_array=self.data.ravel(), deep=True, array_type=dtype
        )
    ...
    def scalarfield(self, data):
        ...
        content1 = wx.StaticText(
            self.settings, -1, "X :", style=wx.ALIGN_CENTER_VERTICAL
        )
        ...
        content2 = wx.StaticText(
            self.settings, -1, "Y :", style=wx.ALIGN_CENTER_VERTICAL
        )
        ...
        content3 = wx.StaticText(
            self.settings, -1, "Z :", style=wx.ALIGN_CENTER_VERTICAL
        )
    ...
    def elevation(self, data):
        ...
        self.x_Offset = wx.TextCtrl(
            self.settings, wx.ID_ANY, size=(45, 27), style=wx.TE_PROCESS_ENTER
        )
        ...
        Sizer0.Add(
            self.x_slider, proportion=1, flag=wx.ALIGN_CENTER_VERTICAL | wx.EXPAND
        )
        ...
        Sizer0.Add(
            self.y_slider, proportion=1, flag=wx.ALIGN_CENTER_VERTICAL | wx.EXPAND
        )
        ...
        Sizer1.Add(
            self.z_slider, proportion=1, flag=wx.ALIGN_CENTER_VERTICAL | wx.EXPAND
        )
    ...
    def x_spacing_onmove(self, evt=None):
        if self.image:
            try:
                ...
                offset_mupltiplier = 1.0
            _, y, z = self.image.GetSpacing()
            val = self.x_slider.GetValue()
            x = (val * offset_mupltiplier) / 100.0
            self.axes.SetTotalLength(
                (self.data.shape[0] - 1) * x,
                (self.data.shape[1] - 1) * y,
                self.data.max() * z,
            )
    ...
    def y_spacing_onmove(self, evt=None):
        if self.image:
            ...
            y = (val * offset_mupltiplier) / 100.0
            self.axes.SetTotalLength(
                (self.data.shape[0] - 1) * x,
                (self.data.shape[1] - 1) * y,
                self.data.max() * z,
    ...
    def isosurface(self, data, isovalue, rendtype):
        ...
        self.surf.GetProperty().SetSpecular(0.4)
        ...
        self.isov_scale = 100.0
        self.isov_slider = wx.Slider(
            self.settings,
            wx.ID_ANY,
            value=isovalue * self.isov_scale,
        ...
        self.opacity = wx.TextCtrl(
            self.settings, wx.ID_ANY, style=wx.SL_HORIZONTAL | wx.TE_PROCESS_ENTER
        )
        ...
        self.sagittal = wx.RadioButton(
            self.settings, wx.ID_ANY, label="sagittal (// to Y,Z)"
        )
        self.axial = wx.RadioButton(self.settings, wx.ID_ANY, label="axial (// to X,Y)")
        self.coronal = wx.RadioButton(
            self.settings, wx.ID_ANY, label="coronal (// to X,Z)"
        )
        self.oblique = wx.RadioButton(
            self.settings, wx.ID_ANY, label="oblique (// to X,Y+Z)"
        )
        ...
        self.reslice = None
        key_doc = wx.StaticText(
            self.settings,
            wx.ID_ANY,
            label="Press '+' and '-' keys to translate the slice",
        )
    ...
    def cpt_isosurf(self, event=None):
        try:
            ...
            raise Plotter3DError(
                "Contour level has wrong format : %s" % self.isov.GetValue()
            )
    ...
    def build_axes(self, noZaxis=False):
        if self.axes is None:
            ...
            if not noZaxis:
                ...
                self.axes.SetTotalLength(
                    self.data.shape[0] - 1, self.data.shape[1] - 1, 0
                )
    ...
    def slice(self):
        ...
        center = [
            self.data.shape[0] / 2.0,
            self.data.shape[1] / 2.0,
            self.data.shape[2] / 2.0,
        ]
        # Matrices for axial, coronal, sagittal, oblique view orientations
        axial = vtk.vtkMatrix4x4()
        axial.DeepCopy(
            (1, 0, 0, center[0], 0, 1, 0, center[1], 0, 0, 1, center[2], 0, 0, 0, 1)
        )

        coronal = vtk.vtkMatrix4x4()
        coronal.DeepCopy(
            (1, 0, 0, center[0], 0, 0, 1, center[1], 0, -1, 0, center[2], 0, 0, 0, 1)
        )

        sagittal = vtk.vtkMatrix4x4()
        sagittal.DeepCopy(
            (0, 0, -1, center[0], 1, 0, 0, center[1], 0, -1, 0, center[2], 0, 0, 0, 1)
        )
        ...
        R = np.array(
            self.ExtractRotMat(self.orientation_matrix)
        )  # extraction de la matrice de rotation depuit la vtk 4x4
    ...
    def R2Eul(self, R, tolerance=1e-5):
        ...
        if sang > fuzz:
            ...
            gamma = 0.0
        if self.almost(beta, 0.0, fuzz):
            alpha, beta, gamma = alpha + gamma, 0.0, 0.0
        elif self.almost(beta, 180.0, fuzz):
            alpha, beta, gamma = alpha - gamma, 180.0, 0.0
        alpha = np.mod(alpha, 360.0)
        gamma = np.mod(gamma, 360.0)
        if self.almost(alpha, 360.0, fuzz):
            alpha = 0.0
        if self.almost(gamma, 360.0, fuzz):
            gamma = 0.0
