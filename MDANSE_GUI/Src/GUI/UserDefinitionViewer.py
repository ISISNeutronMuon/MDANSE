...
class UserDefinitionViewer(wx.Dialog):
    def __init__(self, parent, title="User Definition Viewer", ud=None, editable=True):
        ...
        self._info = wx.TextCtrl(
            mainPanel, wx.ID_ANY, style=wx.TE_MULTILINE | wx.TE_READONLY
        )
    ...
    def on_show_info(self, event=None):
        ...
        if level <= 3:
            self._info.SetValue(
                "Contains the followings definitions: %s" % list(data.keys())
            )
            return
    ...
    def on_delete(self, event):
        ...
        if keycode == wx.WXK_DELETE:
            ...
            if level == 1:
                ...
                targetItemName = str(self._tree.GetItemText(targetItem))
                UD_STORE.remove_definition(
                    targetItemName, sectionItemName, currentItemName
                )
            else:
                return
    ...
    def on_rename(self, event):
        ...
        UD_STORE.set_definition(
            targetItemName, sectionItemName, newItemName, currentItemData.GetData()
        )
...
if __name__ == "__main__":
    app = wx.App(False)
    f = UserDefinitionViewer(
        None,
        ud=["protein_in_periodic_universe.nc", "atom_selection", "sfdfdfsd"],
        editable=True,
    )
    f.ShowModal()
    f.Destroy()
