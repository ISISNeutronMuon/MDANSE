...
class InstrumentResolutionDialog(wx.Dialog):
    def __init__(self, parent=None, nSteps=100, timeStep=1.0):
        ...
        self._omegas = (
            2.0 * np.pi * np.fft.fftshift(np.fft.fftfreq(self._nSteps, timeStep))
        )
    ...
    def build_dialog(self):
        ...
        self._kernelChoice = wx.Choice(
            self, wx.ID_ANY, choices=list(REGISTRY["instrument_resolution"].keys())
        )
...
class InstrumentResolutionWidget(IWidget):
    def add_widgets(self):
        ...
        self._resolution = wx.TextCtrl(
            self._widgetPanel, wx.ID_ANY, value=str(self._configurator.default)
        )
    ...
    def on_set_instrument_resolution(self, event):
        ...
        self._instrumentResolutionDialog = InstrumentResolutionDialog(
            self, nSteps, timeStep
        )
