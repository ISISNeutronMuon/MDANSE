...
class ProjectionWidget(IWidget):
    def add_widgets(self):
        ...
        self._none = wx.RadioButton(
            self._widgetPanel, wx.ID_ANY, label="None", style=wx.RB_GROUP
        )
        ...
        labelAxial = wx.StaticText(
            self._panelAxial, wx.ID_ANY, size=(80, -1), label="Axis vector"
        )
        ...
        labelPlanar = wx.StaticText(
            self._panelPlanar, wx.ID_ANY, size=(80, -1), label="Normal vector"
        )
        ...
        if rb == self._none:
            ...
            try:
                val = (
                    "axial",
                    tuple(
                        [float(v.GetValue()) for v in (self._ax, self._ay, self._az)]
                    ),
                )
            except ValueError:
            ...
            try:
                val = (
                    "planar",
                    tuple([v.GetValue() for v in (self._px, self._py, self._pz)]),
                )
            except ValueError:
                raise ConfigurationError("Invalid value for %r entry" % self.name)
            else:
